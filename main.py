...
def pick_lego(lego):
    if lego.active == False:
        ...
        dexarm.fast_move_to((x,y, 20), feedrate = 100000)
        dexarm.fast_move_to((x,y, -32), feedrate = 100000)
        if dexarm.get_current_position()[:2] == (cXmm, cYmm):
            dexarm.soft_gripper_pick()
            dexarm.fast_move_to((x,y,30), feedrate = 100000)
            dexarm.move_rail(extrude = 400, feedrate = 100000)
            # dexarm.move_to(cc.colourBucket(colour))
            dexarm.fast_move_to((200, 100, 0), feedrate = 100000)
            dexarm.soft_gripper_place()
            dexarm.move_rail(extrude = 0, feedrate = 100000)
            # dexarm.go_home()
            dexarm.fast_move_to((0, 200, 30), feedrate = 100000)
